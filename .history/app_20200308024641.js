                  ...
                  Draft.findById(currentRoom, function(err, data){
                    ...
                    Draft.findById(currentRoom, function(err, data){
                      ...
                      try{
                        ...
                        var vwaPlayer = playerList[playerList.length - 1];
                        var vwaEstimatedPrice = player.vwa;
                        var vwaActualPrice = newPlayer.price;
                        var oversUnders = vwaActualPrice - vwaEstimatedPrice;

                        if(oversUnders < 0) {
                          var text = "Uuunders! " = vwaPlayer + " has been heisted at " + Math.round((((oversUnders * -1)/vwaEstimatedPrice)*100)) + "% unders!";
                        } else if(oversUnders > 0) {
                          var text = "Oooovers! " = vwaPlayer + " is on an inflated pay packet at " + Math.round((((oversUnders)/vwaEstimatedPrice)*100)) + "% overs!";
                        } else {
                          equal
                        }
