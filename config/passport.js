...
module.exports = function(passport) {
    ...
    function(req, email, password, done) {
        ...
        User.findOne({ 'local.email' :  email.toUpperCase() }, function(err, user) {
            // if there are any errors, return the error
            if (err)
                return done(err);
            // check to see if theres already a user with that email
            if (user) {
                return done(null, false, req.flash('signupMessage', email.toUpperCase() + ' is already in use!'));
            } else {
                ...
                newUser.local.name = req.body.name.toUpperCase();
                // save the user
    ...
    })); // Close passport.use('local-signup') function.
    // LOCAL LOGIN
    // we are using named strategies since we have one for login and one for signup
    // by default, if there was no name, it would just be called 'local'
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        User.findOne({ 'local.email' :  email.toUpperCase() }, function(err, user) {
            ...
            if (!user)
                return done(null, false, req.flash('loginMessage', 'No user found: ' + email)); // req.flash is the way to set flashdata using connect-flash
    ...
    })); // Close passport.use('local-login') function.
    ...
    })); // Close passport.use(FacebookStrategy) function.
    ...
    })); // Close passport.use(TwitterStrategy) function.
